    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for t in range(100):
            ...
            delta = reward - Q[state][action] + max(Q[next_state]) - discount_factor * Q[state][action] * max(
                Q[next_state])  # delta is the time-difference error
...
Q = td_control(env, num_episodes, step_size=0.01)
...
if current_env_windy:
    ...
    axs[0, 0].plot(x, np.array(np.array(Q_r)[:, 0]) - np.array(np.array(Q_l)[:, 0]))
    axs[0, 0].set_title('Q(24, r) - Q(24, l) (HDTD)')
    axs[0, 1].plot(x, np.array(np.array(Q_b)[:, 1]) - np.array(np.array(Q_u)[:, 1]))
    axs[0, 1].set_title('Q(25, b) - Q(25, u) (HDTD)')
    axs[1, 0].plot(x, np.array(np.array(Q_l)[:, 2]) - np.array(np.array(Q_u)[:, 2]))
    axs[1, 0].set_title('Q(31, l) - Q(31, u) (HDTD)')
    axs[1, 1].plot(x, np.array(np.array(Q_u)[:, 3]) - np.array(np.array(Q_l)[:, 3]))
    axs[1, 1].set_title('Q(32, u) - Q(32, l) (HDTD)')
    ...
    axs[0].set_title('Difference Q(21, u) - Q(21, r) (HDTD)')
    axs[1].plot(x, np.array(Q_l)[:, 1] - np.array(Q_u)[:, 1])
    axs[1].set_title('Difference Q(9, l) - Q(9, u) (HDTD)')
    fig.show()
    ...
    axs[0].set_title('Q(s=21) HDTD: Gridworld')
    ...
    axs[1].set_title('Q(s=9) HDTD: Gridworld')
    plt.legend()
    fig.show()
