# -*- coding: utf-8 -*-
"""
Created on Wed Nov 16 11:22:35 2022
...
for _ in range(num_seeds):
    ...
    q_b_s.append(expu_b)
    q_l_s.append(expu_l)
q_u_s, q_r_s, q_b_s, q_l_s = np.array(q_u_s), np.array(q_r_s), np.array(q_b_s), np.array(q_l_s)

final_q_u, final_q_r, final_q_b, final_q_l = [], [], [], []
...
final_q_u, final_q_r, final_q_b, final_q_l = np.array(final_q_u), np.array(final_q_r), np.array(final_q_b), np.array(final_q_l)
# ------------------------------------------------------------------------------------------------
...
if current_env_windy:
    ...
    if isSoftmax:
        fig.suptitle('SophEU: Softmax' + ' alpha: ' + str(alpha))
    else:
        fig.suptitle('SophEU: \u03B5-greedy' + ' (\u03B5=' + str(epsilon)+')')
    fig.show()
    ...
    axs[1, 1].legend()
    if isSoftmax:
        fig.suptitle('SophEU: Using Softmax' + ' alpha: ' + str(alpha))
    else:
        fig.suptitle('SophEU: \u03B5-greedy' + ' (\u03B5=' + str(epsilon)+')')
    fig.show()
    ...
    axs[1].set_title('Q(s=9)-' + env_ID)
    plt.legend()
    if isSoftmax:
        fig.suptitle('SophEU: Softmax' + ' (alpha=' + str(alpha) + ') , step_size=' + str(step_size))
    else:
        fig.suptitle('SophEU: \u03B5-greedy' + ' (\u03B5=' + str(epsilon)+')' + ', step_size=' + str(step_size))
    fig.show()
