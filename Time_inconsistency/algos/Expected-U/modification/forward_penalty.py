...
isSoftmax = False
if isSoftmax:
    alpha = .1  # The noise parameter that modulates between random choice (=0) and perfect maximization (=\infty)
else:
    epsilon = .2
...
def td_control(env, num_episodes, isSoftmax):
    ...
    agent = make_policy(expUtility, env.action_space.n, isSoftmax)
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for t in range(100):
            ...
            u = discount(t) * (state_utility + (-1) * penalty)
            if isSoftmax:  # softmax
...
if current_env_windy:
    ...
    for r_ in range(nr):
        ...
        for c_ in range(nc):
            if r_ * nc + c_ == 24 or r_ * nc + c_ == 31:
                a = 0 if expUtility[0][r_ * nc + c_][0] > expUtility[r_ * nc + c_][0][1] else 1
                row.append(a)
            else:
                row.append(np.argmax(expUtility[0][r_ * nc + c_]))
        print(row)
    ...
    if isSoftmax:
        fig.suptitle('Using Softmax' + ' alpha: ' + str(alpha))
    else:
        fig.suptitle('Using Eps Soft-greedy' + ' epsilon: ' + str(epsilon))
    fig.show()
