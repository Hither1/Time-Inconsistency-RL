...
algo_ID = 'bwd' # 'MC', 'bwd-rev', 'sophEU', 'Fedus'
if algo_ID == 'sophEU':
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for t in range(len(episode) - 2, -1, -1):
            ...
            for i in range(len(G)):
                Q_g[s][a][G[i]] += lr * (r + G[i] * Q_g[nextstate][policy[next_state]][G[i]] - Q_g[s][a][G[i]])
            Q[s][a] = np.sum([Q_g[s][a][G[i]][-1] for i in range(len(G))])
            '''
...
for seed_ in range(num_seeds):
    ...
    # update docs
    df = pd.read_csv('results/' + title + '.csv', header = None)
...
if current_env_windy:
    ...
    #fig.show()
    fig.savefig('stochastic_graphs/' + title + '.png')
