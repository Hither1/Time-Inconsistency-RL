...
if env_ID == 'D':   
    ...
    G_size = 1000 #1000 # 1000 better, 200 for fair comparison on the num of vf
...
algo_ID = 'bwdFedus' # 'MC', 'bwd-rev', 'sophEU', 'Fedus'
if algo_ID == 'sophEU':
...
def td_control(env, num_episodes, isSoftmax, step_size_1, step_size_2, G):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        if i_episode % 1000 == 0:
            ...
            #print(np.round(Q[21], 2))
            #print(np.round(Q[9], 2))
            sys.stdout.flush()
            
            # For lr = .5, there is persisting bias from Q/V table, 
            # but relative ordering is ok s.t. -> is chosen at about 
            # similar i*_21 to bwd-ori.
            
            # What may not be desirable here is the reliance on bias(Q(s, a0))
            # ~ bias(Q(s, a1)). Note that if this is not the case, there will
            # be a 'flip' in s' preference over {a0, a1} bcz of the bias.
            
            '''
            FOR FEDUS
            # [ok, worse than bwd-ori cz of the high bias] Check lr.4 50-seeds graph & i*_21 S
            # [skip, D .5 not better than D .4] Run S (lr.5) @LPTOP.
            # [ok, abt 11k still] Check D (lr.5) @DESKTOP: i*_21 for 50 runs (~6k, not 11k like lr = .4?)
            # [skip] Check S (lr.5) @LPTOP.
            
            # HPC: Run 10 runs each, D, S x (.4, [skip] .5). Fix directory + dependency.'''
...
for seed_ in range(num_seeds):
    ...
    print('running_time:', time.time()-time0)
