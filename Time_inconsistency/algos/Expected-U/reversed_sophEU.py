...
for _ in range(50):
    ...
    for i in range(num_episodes - 1, 0, -1):
        if expu_r[i][0] >= expu_u[i][0] and expu_r[i - 1][0] < expu_u[i - 1][0]:
            i_21.append(i)
            break
    for i in range(num_episodes - 1, -1, -1):
        if expu_l[i][1] >= expu_u[i][1] and expu_l[i - 1][1] < expu_u[i - 1][1]:
            i_9.append(i)
            break
    q_u_s.append(expu_u)
    q_r_s.append(expu_r)
...
df.to_csv("../../results/reversed/fwd/V_values_" + str(step_size) + ".csv")
...
x = [i for i in range(1, 1 + num_episodes)]
...
axs[0].fill_between(x, final_q_b[:, 21] - final_q_b[:, 21 + 24], final_q_b[:, 21] + final_q_b[:, 21 + 24], alpha=0.2)
#print("(21, below)", np.array(q_b)[:, 0][-1])
axs[0].plot(x, final_q_l[:, 21], label='l')
axs[0].fill_between(x, final_q_l[:, 21] - final_q_l[:, 21 + 24], final_q_l[:, 21] + final_q_l[:, 21 + 24], alpha=0.2)
#print("(21, left)", np.array(q_l)[:, 0][-1])
axs[0].set_title('Q(s=21)')
axs[0].legend()
axs[1].plot(x, final_q_u[:, 9], label='u')
axs[1].fill_between(x, final_q_u[:, 9] - final_q_u[:, 9 + 24], final_q_u[:, 9] + final_q_u[:, 9 + 24], alpha=0.2)
axs[1].plot(x, final_q_r[:, 9], label='r')
axs[1].fill_between(x, final_q_r[:, 9] - final_q_r[:, 9 + 24], final_q_r[:, 9] + final_q_r[:, 9 + 24], alpha=0.2)
axs[1].plot(x, final_q_b[:, 9], label='b')
axs[1].fill_between(x, final_q_b[:, 9] - final_q_b[:, 9 + 24], final_q_b[:, 9] + final_q_b[:, 9 + 24], alpha=0.2)
axs[1].plot(x, final_q_l[:, 9], label='l')
axs[1].fill_between(x, final_q_l[:, 9] - final_q_l[:, 9 + 24], final_q_l[:, 9] + final_q_l[:, 9 + 24], alpha=0.2)
axs[1].set_title('Q(s=9)')
fig.suptitle("SophEU Deterministic, step_size=" + str(step_size)+ ' (\u03B5=' + str(epsilon) + ')')
plt.legend()
plt.show()
